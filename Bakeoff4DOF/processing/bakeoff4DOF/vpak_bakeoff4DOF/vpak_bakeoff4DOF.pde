...
void draw() {
  ...
  for (int i=trialIndex; i<trialCount; i++) // reduces over time
  {
    ...
    if (trialIndex==i)
      if (checkForSuccess()) {
        stroke(0, 255, 0, 192); //set color to semi translucent green if in correct position
      } else {
        stroke(255, 0, 0, 192); //set color to semi translucent red if not yet in correct position
      } else
...
void mouseMoved() {
  if (mouseInRect()) { // Change cursor to reflect what type of movement can be done
    cursor(MOVE);
...
boolean mouseInRect() {
  return dist(logoX, logoY, mouseX, mouseY)<logoZ/2;
}
